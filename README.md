...
# Experimental: Automatic Clipping

<img width="1100" alt="auto-clipping" src="https://user-images.githubusercontent.com/91582112/137959073-47467d87-b78a-4819-8bfb-dc0d7bd1ff33.png">

When you have a feature that should be _clipped_ by another feature (e.g. Hair should not overflow out of a helmet),you can use a _Folder Trait_ with multiple files inside. (see above section on required images).

To use a clipping mask, you must first, include a file with `-CLIP` in the file name, along with the artwork.
![image](https://user-images.githubusercontent.com/91582112/137957969-249791a5-932c-4ff5-a557-266d950f83cf.png)

Then, this is where it get's tricky:
Because of the way clipping clips _everything_ underneath it, you need to setup your layer configuration with this in mind. First, add elements that _can_ be clipped by a `-CLIP` element.
e.g., in the following structure, the head, hair, and eyes all go _under_ the helmet and are OK to be clipped by the clipping mask. If the shirt was included above (as usuaul) it would also be clilpped–NOT what we want.
...
_For any layers that should go *under* the clipped items:_
You will need to do two things:

1.  add them _after_ the layer that includes a clipping mask
2.  specify the `destination-over` blend mode (this puts it _behind_ the above layers)
...
For layers that _can stack_ on top of the clipped images, like accessories, you may define them below, as usual.
...
⚠️ The layers directory in config is set to the clipping example

```
const layersDir = path.join(basePath, "/clipping-example");
```

Change this to the folder that includes your images.
